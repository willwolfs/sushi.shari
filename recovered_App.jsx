...
const roleTabs = {
  Administrador: ['POS', 'Caja', 'Reportes', 'Historial', 'Usuarios'],
  Cajero: ['POS', 'Caja', 'Historial'],
  Cocina: ['Historial'],
};

// Removido bcrypt.hashSync() para prevenir fallos en el navegador
const defaultUsers = [
  {
    id: 'USR-1',
...
function App() {
  ...
  const getDocumentType = (doc) => {
    const clean = doc.replace(/\D/g, '');
    return clean.length === 8 ? 'DNI' : clean.length === 11 ? 'RUC' : 'DOCUMENTO';
  };

  const generateReceipt = (sale) => {
    const date = new Date(sale.timestamp).toLocaleString('es-PE');
    const docType = getDocumentType(sale.clientDNI);
...
${Math.abs(diff) < 0.01 ? '✓ CAJA BALANCEADA CORRECTAMENTE' : diff > 0 ? '⚠ SOBRANTE EN CAJA' : '⚠ FALTANTE EN CAJA'}`;
  ...
  const cashSales = todayPaidSales
                ...
                <div className="section">
                  ...
                  {clientDNI && (
                    <div className="client-info">
                      <small>✓ {getDocumentType(clientDNI)}: {clientDNI.replace(/\D/g, '')}</small>
                    </div>
                  )}
                </div>
            ...
            <div className="financial-grid">
              <div className="financial-box">
                ...
                <div className="method-list">
                  ...
                  <div className="method-row"><span>ATM Tarjeta Débito</span><strong>S/ {paymentBreakdown.debit.toFixed(2)}</strong></div>
              ...
              <div className="modal" onClick={() => setSelectedSale(null)}>
                <div className="modal-box" onClick={(e) => e.stopPropagation()}>
                  ...
                  <div className="modal-body">
                    <p><span>Mesa:</span> <strong>{selectedSale.table}</strong></p>
                    <p><span>Fecha:</span> <strong>{new Date(selectedSale.timestamp).toLocaleString('es-PE')}</strong></p>
                    <p><span>Método:</span> <strong>{selectedSale.paymentMethod}</strong></p>
                    <p><span>Estado:</span> <strong className={selectedSale.verified ? 'verified' : 'pending'}>{selectedSale.verified ? '✓ Verificado' : '⏳ Pendiente'}</strong></p>
                    <hr />
                    <h4>Items ({selectedSale.items.length})</h4>
                    {selectedSale.items.map((item, i) => (
                      <p key={`modal-item-${selectedSale.id}-${item.id}-${i}`}>
                        {item.name} x{item.quantity} = S/ {(item.quantity * item.price).toFixed(2)}
                      </p>
                    ))}
                    <hr />
                    <p><span>Subtotal:</span> <strong>S/ {selectedSale.subtotal.toFixed(2)}</strong></p>
                    {selectedSale.discountValue > 0 && <p><span>Descuento:</span> <strong>-S/ {selectedSale.discountValue.toFixed(2)}</strong></p>}
                    <p><span>IGV (18%):</span> <strong>S/ {selectedSale.tax.toFixed(2)}</strong></p>
                    <p className="total"><span>Total:</span> <strong>S/ {selectedSale.total.toFixed(2)}</strong></p>
                  </div>
          ...
          <div className="modal-box client-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                />
                {clientDNI && (
                  <small className={validateDocument(clientDNI) ? 'success' : 'error'}>
                    {validateDocument(clientDNI)
                      ? `✓ ${getDocumentType(clientDNI)} válido`
            ...
            <div className="modal-actions">
              <button
                onClick={() => {
                  if (clientName.trim() && validateDocument(clientDNI)) {
                    setShowClientModal(false);
                  } else {
                    alert('Por favor complete los datos correctamente');
                  }
                }}
                disabled={!clientName.trim() || !validateDocument(clientDNI)}
